...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # Browser Integration (HTTP server for extension)
        try:
            ...
            self.http_server = start_http_server(self, port=8765)
            if self.http_server:
                logger.info("Browser integration server started successfully on port 8765")
            else:
                logger.warning("Failed to start browser integration server - extension won't work")
        except Exception as e:
            if hasattr(self, "logger"):
                logger.error(f"Browser integration startup error: {e}")
            else:
