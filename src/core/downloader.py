...
class Downloader:
    ...
    def __init__(
        ...
        self.config = {}  # Config dict for settings like concurrent_fragments
    ...
    def download_stream_ydl(self):
        ...
        if self.format_info and self.format_info.get("is_playlist"):
            ...
            # Single video: Use title template
            output_template = os.path.join(self.save_dir, "%(title)s.%(ext)s")
            cmd.extend(["-o", output_template])

        # Concurrent fragments optimized for Python 3.14
        # Even with GIL, modern scheduler handles 64 threads better
        concurrent_fragments = self.config.get("concurrent_fragments", 64)
        cmd.extend(["-N", str(concurrent_fragments)])

        # CRITICAL: If user selected "single", add --no-playlist
        # This prevents yt-dlp from downloading entire Mix/Radio playlists
        if self.format_info and not self.format_info.get("is_playlist"):
            cmd.append("--no-playlist")
            self.log("🎯 Single video mode: --no-playlist added")
    ...
    def start(self):
        ...
        # For streaming sites (YouTube, etc.), always use yt-dlp
        if self._check_ytdlp():
            self.log("🔀 Using yt-dlp for streaming download")
            success = self.download_stream_ydl()
            if self.completion_callback:
                self.completion_callback(success, self.filename)
            return
