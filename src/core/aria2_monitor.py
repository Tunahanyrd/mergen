...
class Aria2Monitor:
    ...
    def __init__(self, rpc_url: str = "http://localhost:6800/jsonrpc", rpc_secret: str = "mergen_secret"):
    ...
    async def _optimize_download(self, gid: str, status: dict):
        ...
        try:
            ...
            int(status.get("completedLength", 0))
            ...
            if download_speed < potential_speed * 0.5 and num_connections < 32:
                ...
                await self._rpc_call(
                    ...
                    [
                        ...
                        gid,
                        {"max-connection-per-server": str(new_max), "split": str(new_max)},
                    ],
                )
        ...
        try:
            result = await self._rpc_call(
                ...
                [
                    f"token:{self.rpc_secret}",
                    gid,
                    ["status", "totalLength", "completedLength", "downloadSpeed", "connections", "numSeeders"],
                ],
            )
            return result
        except Exception as e:
