...
class DownloadManager:
    ...
    def save_state(self):
        ...
        try:
            state = {did: self._serialize_download(item) for did, item in self.downloads.items()}
    ...
    def load_state(self):
        ...
        try:
            ...
            self.downloads = {did: self._deserialize_download(data) for did, data in state.items()}
    ...
    def _deserialize_download(self, data: dict) -> DownloadItem:
        ...
        if download_type == DownloadType.PLAYLIST:
            ...
            videos = [self._deserialize_video(v) for v in data.get("videos", [])]
    ...
    def get_pending_downloads(self) -> list[DownloadItem]:
        ...
        return [item for item in self.downloads.values() if item.status == DownloadStatus.PENDING]

    def get_active_downloads(self) -> list[DownloadItem]:
        """Get currently downloading items"""
        return [item for item in self.downloads.values() if item.status == DownloadStatus.DOWNLOADING]
